...
import com.habit.entity.Xuexijihua;
import com.habit.mapper.XuexijihuaMapper;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.util.StringUtils;
import org.springframework.web.bind.annotation.*;

@RestController
@RequestMapping("/api/xuexijihua")
public class XuexijihuaController {

    @Autowired
    private XuexijihuaMapper xuexijihuaMapper;
    ...
    public R list(@RequestParam(defaultValue = "1") int page,
        ...
        QueryWrapper<Xuexijihua> qw = new QueryWrapper<>();
        ...
        qw.orderByDesc("id");
        Page<Xuexijihua> p = xuexijihuaMapper.selectPage(new Page<>(page, limit), qw);
        return R.page(p.getRecords(), p.getTotal());
    }

    @GetMapping("/info/{id}")
    public R info(@PathVariable Long id) {
        return R.ok(xuexijihuaMapper.selectById(id));
    }

    @PostMapping("/add")
    public R add(@RequestBody Xuexijihua entity) {
        ...
        xuexijihuaMapper.insert(entity);
        return R.ok(entity);
    }

    @PutMapping("/update/{id}")
    public R update(@PathVariable Long id, @RequestBody Xuexijihua entity) {
        entity.setId(id);
        entity.setKaishiriqi(toDateOnly(entity.getKaishiriqi()));
        entity.setJieshushijian(toDateOnly(entity.getJieshushijian()));
        xuexijihuaMapper.updateById(entity);
        return R.ok("修改成功");
    }

    @DeleteMapping("/delete/{id}")
    public R delete(@PathVariable Long id) {
        xuexijihuaMapper.deleteById(id);
        return R.ok("删除成功");
    }
